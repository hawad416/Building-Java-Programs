...
and prints the numbers in the range from min to max inclusive in a square pattern. The square 
...
Each line prints a different permutation of this sequence. The first line begins with min, the second 
line begins with min + 1, and so on. When the sequence in any line reaches max, it wraps around back to min.
...
public static void printSquares(int min, int max) {
		...
		System.out.println();	
			...
			while(count !=max-originalMin) {

					
					for(int i=min; i<=max; i++) {
						System.out.print(i);
					}
					for(int j=originalMin;j<=min-1; j++) {
						System.out.print(j);
					}
					System.out.println();
