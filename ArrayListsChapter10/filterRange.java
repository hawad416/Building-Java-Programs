/*Write a method filterRange that accepts an ArrayList of integers 
and two integer values min and max as parameters and removes
all elements whose values are in the range min through max (inclusive) from the list.
...
the list remains empty. You may assume that the list is not null.
*/
...
public static void filterRange(ArrayList<Integer> a, int min, int max) {
		...
		for(int i = 0; i<a.size(); i++) {
			if(a.get(i) == min || a.get(i) == max) {
				a.remove(i);
				i--;
			}else if(a.get(i) > min && a.get(i) < max) {
				a.remove(i);
				i--;
			}
